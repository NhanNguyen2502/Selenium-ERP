...
public class CreatePaymentAccountWithCashTypeTest extends BaseSetup {
    ...
    public void createBankAccountWithValidInformation_then_createSuccess(@Optional("English") String language) {
        ...
        validateHelpers.waitForLoadJs();
        createPaymentAccount.selectTypeOfPaymentAccount(GetTypeOfPaymentAccountHelper.getCashType(language));
    ...
    @Test(priority = 0)
    @Parameters({"language"})
    public void createBankAccountWithOpeningBalance_then_createSuccess(@Optional("English") String language) {
        validateHelpers.waitForLoadJsLoginPage();
        signInPage.verifylanguage(language);
        validateHelpers.waitForLoadJs();
        signInPage.login(PropertiesHelper.getValue("email"), PropertiesHelper.getValue("password"));
